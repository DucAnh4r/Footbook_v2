...
import { acceptFriendshipService, declineFriendshipService, createFriendshipService } from '../../../services/friendService';
...
const FriendRequestItem = ({
  ...
  return (
    <div className={`${styles.content} ${isSelected ? styles.selected : ''}`} onClick={handleClick}>
      <Row align="middle" style={{ cursor: 'pointer' }}>
        ...
        <Col span={19}>
          <Row className={styles['flex-between']}>
            ...
            {itemType === 'request' ? <span className={styles['date']}>{formattedDate}</span> : <span className={styles['mutual-friends']}>{user.mutual_friends_count} bạn chung</span>}
          </Row>

          <Row className={styles['flex-between']} style={{ marginTop: '8px' }}>
            {itemType === 'request' ? (
              <>
                <Button onClick={handleAccept} className={`${styles['button']} ${styles['confirm-button']}`} type="primary">
                  Xác nhận
                </Button>
                <Button onClick={handleDecline} className={`${styles['button']} ${styles['delete-button']}`}>
                  Từ chối
                </Button>
              </>
            ) : (
              <Button onClick={handleSendRequest} className={`${styles['button']} ${styles['add-button']}`} type="primary">
                Thêm bạn bè
              </Button>
            )}
          </Row>

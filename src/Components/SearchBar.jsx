...
const SearchBar = () => {
  ...
  const performSearch = async keyword => {
    ...
    try {
      ...
      const normalizedUsers = users.map(user => ({
        ...
        avatar_url: user.avatar_url || user.profilePictureUrl || 'https://https://cdn.vectorstock.com/i/500p/29/53/gray-silhouette-avatar-for-male-profile-picture-vector-56412953.jpg.com/50',
        profilePictureUrl: user.avatar_url || user.profilePictureUrl || 'https://https://cdn.vectorstock.com/i/500p/29/53/gray-silhouette-avatar-for-male-profile-picture-vector-56412953.jpg.com/50'
      }));
  ...
  const handleClickOutside = event => {
    if (resultsRef.current && !resultsRef.current.contains(event.target) && searchBarRef.current && !searchBarRef.current.contains(event.target)) {
      setShowSuggestions(false);
    }
  };
          ...
          {isLoading ? (
            ...
            <>
              {suggestions.slice(0, 5).map(user => (
                <div key={user.id || user.userId} className={styles.suggestionItem} onClick={() => handleSuggestionClick(user.id || user.userId)}>
                  <Avatar src={user.avatar_url || user.profilePictureUrl} size="small" className={styles.avatar} />
              ...
              <div className={styles.viewAllContainer} onClick={handleViewAll}>
                <Button type="link" className={styles.viewAllButton} icon={<FaChevronRight style={{ fontSize: '12px' }} />}>
